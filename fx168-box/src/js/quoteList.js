		 ...
		 for (var i = 0; i < $(".find_nav_list li").length; i++) {
		   if (index==0) {
		       sessionStorage.tabUrlKey="wh";//要闻
		        ...
		        sessionStorage.tabUrlKey="gjhj";//外汇
		   }else if(index==2){
   ...
   function getTabListData(data) {
   	...
   	var hq_time=setInterval(function(){
    	...
    	for (var i = 0; i < dataListArr.length; i++) {    		
    		hqListResult+='<a class="hq_List" data-key='+dataListArr[i].key+' href="hq_inner2.html">'    						
								+'<div class="hq_nk">'
    	...
    	// 判断背景颜色
    	for (var i = 0; i < $('.rangeShow1').length; i++) {
		    ...
		    $.each(resArr,function(key,value){
		     ...
		     for (var i = 0; i < $('.hq_List').length; i++) {
	     		...
	     		if ($('.hq_List').eq(i).attr("data-key")==key1) {
	     			...
	     			if(data[5]>0 && data[0]!=$('.hq_List .hq_tradePrice span').eq(i).html()){
	     			...
	     			}else if(data[5]<0 && data[0]!=$('.hq_List .hq_tradePrice span').eq(i).html()){
	     				$('.hq_List .hq_tradePrice span').eq(i).css({
