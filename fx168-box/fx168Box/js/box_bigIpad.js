...
function chartAjax () {
    $.ajax({
        ...
        success: function (data){
            ...
            var myObj=[{
                name: '季',
                data: [{ name: '利多', b: '利空', color: '#ee5b5b', c: '#2fca7b', y: parseFloat(datasj[0]), x: parseFloat(datasj[1]), selected: true },
                       { name: '利空', b: '利多', color: '#2fca7b', c: '#ee5b5b', y: parseFloat(datasj[1]), x: parseFloat(datasj[0]), selected: true }],
                size: '110%',
                innerSize: '80%',
            }, {
                name: '月',
                data: [{ name: '利多', b: '利空', color: '#ee5b5b', c: '#2fca7b', y: parseFloat(datasm[0]), x: parseFloat(datasm[1]), selected: true },
                       { name: '利空', b: '利多', color: '#2fca7b', c: '#ee5b5b', y: parseFloat(datasm[1]), x: parseFloat(datasm[0]), selected: true }],
                size: '90%',
                innerSize: '70%',
            },{
                name: '周',
                data: [{ name: '利多', b: '利空', color: '#ee5b5b', c: '#2fca7b', y: parseFloat(datasw[0]), x: parseFloat(datasw[1]), selected: true },
                       { name: '利空', b: '利多', color: '#2fca7b', c: '#ee5b5b', y: parseFloat(datasw[1]), x: parseFloat(datasw[0]), selected: true }],
                size: '70%',
                innerSize: '70%',
            }]
            // console.log(myObj);
...
function getDataList(data) {
        ...
        res+='<ul class="infoList_wrap infoList_wrap'+i+'" zb_code="'+arr[i].zb_code+'" pubdatetime="'+arr[i].pub_datetime+'" tabIsTrue="'+true+'" isjt="'+true+'">'
                +'<li class="infoList_wrap_lis">'
                    ...
                    +'<div class="infoList_li_wrap">'                  
                        ...
                        +'</div>'
                        +'<div class="jt">'
                            +'<img src="img/box_bigIpad/qiehuanzhankai@2x.png" />'
                        +'</div>'
                    +'</div>'
                +'</li>'
            ...
            +'<div class="bar_chart">'
                +'<div class="country_wrap">'
                    +'<img src="img/box_bigIpad/biaoshi@2x.png" />'
    ...
    $(".infoList_wrap").click(function(e){
        ...
        if (isjt=="true") {
            ...
            $(this).find('.jt img').attr("src",commonImgSrc+"/shouqi@2x.png")
                      
        }else{
            $(this).attr("isjt",true)
            $(this).find('.jt img').attr("src",commonImgSrc+"/qiehuanzhankai@2x.png")
        }
    });
    initTabInfo2();
                ...
                result+='<a href="box_ipadNews.html?newsId='+organizationArr[i].cmsid+'" class="organizationWrap" cmsid="'+organizationArr[i].cmsid+'">'
                            +'<li class="organization_title">'
                                +'<span class="point_wrap">'
                                    +'<img class="point" src="img/box_bigIpad/point@2x.png">'
                                +'</span>'
                ...
                for (var i = 0; i < transactionArr.length; i++) {
                    ...
                    result+='<ul class="transactionList_wrap">'
                                +'<li class="transactionList_tx">'
                                    +'<img class="head'+loginCode+' _head" src="../img/touxiang@2x.png" />'
                                +'</li>'
