...
$(function() {
  // returnTokenUserId("", "1918508987381");
  getTokenUserId();
  // jquery tab动效
  // tabAnimate ();
  // 滚动定位tab
          ...
          success: function(data) {
            ...
            for (var i = 0; i < arr.length; i++) {
              ...
              result += ' <div class="teachInfo clearfix" >' +
                ...
                ' <p class="descInfos">' + desc + '</p>' +
                '<span class="seeMore tageList">' + '查看更多' + '</span>' +
                '</div>' +
                '</div>';
            }
  ...
  $(".detailInfoChild").on("click", ".tageList", function() {
    showMore.call(this, $(this));
  });
...
function getBasicData(data) {
  ...
  var shareJson = {
    ...
    shareTitle: tile,
    shareUrl: baseUrl + "/app/shareTrainingInner.html?brokerUserId=" + brokerUserId,
    shareImage: logoUrl,
    shareContent: "给你推荐一个培训机构，去看看大家对它的评价吧"
  };
...
function androidShare(shareJson) {
  if(window.SysClientJs) window.SysClientJs.shareJs(shareJson);
