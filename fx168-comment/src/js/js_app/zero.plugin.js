...
// 获取sortIndex 值
var sortIndex = window.localStorage ? localStorage.getItem("sortIndex") : 0;
/**
 *
 *获取经纪商基本信息ajax方法
 *
  ...
  if (res.data && res.data.list) {
    ...
    for (var i = 0; i < list.length; i++) {
      str += '<li><a href="javascript:;" data-userid="' + list[i].userId + '">\
          ...
          <img class="lazy" data-original="' + list[i].logo + '"  src="' + require("../../img/img_app/liebiao@2x.png") + '"></a></li>'
    }
  }
  return str;
}
...
function ComQuestion(res) {
  ...
  for (var i = 0; i < list.length; i++) {
    str += '<section class="compro-list" data-qslist="true">\
        <aside class="com_qs">\
          <img src="' + require("../../img/img_app/icon_cp.png") + '" alt="">\
...
Tip.prototype.upHtml = function() {
  var str = '<section class="com_tip_updata" style="display:block"><div class="wrap_tp">\
      <ins class="close_tip"><img src="' + require("../../img/img_h5_v.2.0/guanbi@2x.png") + '"></ins>\
...
function masuShare() {
  var shareObj = {
    ...
    shareImage: "https://m.fx168.com/img/img_download/jy_fx168_mobile_load_logo.png",
...
function getPiratedPlatform(res) {
  ...
  for (var i = 0; i < list.length; i++) {
    str += '<li class="over_h">\
          ...
          <img class="warn_mark" src="' + require("../../img/img_app/warning2x.png") + '" alt="">\
        </li>'
  }
  ...
  for (var i = 0; i < list.length; i++) {
    str += '<li>\
      <div class="title-license clearfix">\
        ...
        <aside class="right icon"><img src="' + require("../../img/img_app/paixu@2x.png") + '" alt=""></aside>\
      </div>\
...
function emitEvent(res) {
  ...
  this.len = Object.keys(this.leftArr);
  return this;
}

function computedAttr() {
  var classArr = new Object();
  var len = this.len.length;
  for (var i in this.leftArr) {
    if (parseFloat(this.rightArr[i].percent) > parseFloat(this.leftArr[i].percent)) {
      classArr[i] = {
        right: 'up',
        left: 'down',
      }
    } else if (parseFloat(this.rightArr[i].percent) < parseFloat(this.leftArr[i].percent)) {
      classArr[i] = {
        right: 'down',
        left: 'up'
      }
    } else {
      if (this.leftArr[i].percent == 0) {
        classArr[i] = {
          right: 'down',
          left: 'down'
        }
...
function directionArr(arr, direction) {
  var str = '';
  if (arr && arr instanceof Object) {
    for (var i in arr) {
      str += '<li style="width:' + (arr[i].percent <= 20 ? 20 : arr[i].percent) + '%;' + (arr[i].num == "" ? "visibility: hidden;" : "") + '"><ins class="' + this.classArr[i][direction] + '">' + (arr[i].num ? arr[i].num : "0.00") + '</ins></li>';
    }
  } else {
...
function defindOnly() {
  defineSetGet.call(this, "leftArr");
  defineSetGet.call(this, "rightArr");
  return this;
}

function defineSetGet(attr) {
  var that = this;
  var arrmap = that[attr];
    ...
    method: function() {
      ...
      if (code != 0) {
        this.ajaxContrast('/comment/getCompareInfoByBrokerUserId.json', function(res) {
          ...
          complex.rightArr = {
            euSpread: {
              num: comList.euSpread,
              percent: comList.euSpreadPercent
            },
            outspread: {
              num: comList.outspread,
              percent: comList.outspreadPercent
            },
            hardware: {
              num: comList.hardware,
              percent: comList.hardwarePercent
            },
            comment: {
              num: comList.comment,
              percent: comList.commentPercent
            },
            complex: {
              num: comList.complex,
              percent: comList.complexPercent
            }
          }
        }, {
          brokerUserId: code
...
//新内页跳转URL
var navigatorList = ["componyIntro.html", "licenseInfo.html", "platform.html", "tradeInfo.html",
  "pointTable.html", "rollovers.html", "exitEntry.html", "realTest.html"
];
$("#bateNavigator li").each(function(i) {
  linkClick.call(this, $(this), i);
})

function linkClick(elem, i) {
  elem.click(function() {
    location.href = navigatorList[i] + "?brokerUserId=" + parseInt(brokerUserId);
  })
}
