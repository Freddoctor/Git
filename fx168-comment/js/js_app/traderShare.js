$(function(){
    // returnTokenUserId("","1884070301682");
    
    window.brokerUserId = getQueryString("brokerUserId");

    //window.brokerUserId ="1888481071581";
    ajaxGetBasicData();
    loadTraderScript();
    // returnTokenUserId();
    // getTokenUserId ();
        ...
        $.ajax({
               ...
               success:function (data) {
                  ...
                  for (var i = 0; i < arr.length; i++) {
                    siteImg+='<a class="a_site" href="'+arr[i].site+'">'
                                  // +'<div class="img_zzc">'+'</div>'
                                  +'<img class="logo_site lazy" data-original="'+arr[i].logo+'" src="../img/img_app/benqi@2x.png" />'+'<br/>'
                       ...
                       if(dataList==""||dataList==null||dataList.length==0){
                           if(pageNo==1){
                             var  noDataImg='<div class="noImgWrap">'
                                       +'<img src="../img/img_app/zwdp@2x.png" />'
                                       +'<p>'+"该交易社区暂无点评"+'<p/>'
...
function getBasicData(data) {
    ...
    if (typeof arrList!=undefined||arrList!=null||arrList!="") {
        ...
        if (arrList.isVip==1) {
            $('.head_wrap').css('background-image','url(../img/img_app/select-bg.png)');
            var flag="";
            ...
            $('.title_bar_isVip').html(flag);
             ...
             $('.head_wrap').css('background-image','url(../img/img_app/@2x.png)');
